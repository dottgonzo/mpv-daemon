            ...
            if (!that.daemonized) {
                ...
                try {
                    ...
                    if (that.noaudio) { // todo demuxer-readahead-packets=300 separate
                        mpv = spawn("mpv", ["--idle", "--really-quiet", "--loop=force", "--rtsp-transport=lavf", "--keep-open=always","--cache=yes", "--cache-pause", "--no-audio", that.socketconf + "=" + that.socketfile], { detached: true, stdio: "ignore" })
                    } else if (options) {
